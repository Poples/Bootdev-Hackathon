...
class GameUI:
    ...
    def draw_damage_flash(self):
        ...
        # Calculate alpha: fade in and then fade out
        if elapsed < self.flash_duration / 2:
            alpha = int(255 * (elapsed / (self.flash_duration / 2)))  # fade in
        else:
            alpha = int(255 * ((self.flash_duration - elapsed) / (self.flash_duration / 2)))  # fade out

        overlay = pygame.Surface(self.screen.get_size(), pygame.SRCALPHA)
        overlay.fill((255, 0, 0, alpha))  # Red with alpha
        self.screen.blit(overlay, (0, 0))

    def draw_game_ui(self,screen, font, game_state,
    ...
    #
        ...
        pygame.draw.rect(self.screen, (50, 0, 50), xp_bar_rect)  # Dark purple background
        pygame.draw.rect(self.screen, (160, 32, 240), (xp_bar_rect.x, xp_bar_rect.y, bar_width * xp_ratio, bar_height))
