...
def initial_zombie_spawn(gs):
    ...
    for i in range(5):  # Start with 5 zombies
        ...
        zombie_x = max(50, min(config.MAP_PIXEL_WIDTH - 50, zombie_x))
        zombie_y = max(50, min(config.MAP_PIXEL_HEIGHT - 50, zombie_y))
...
def get_tile_x_y(player):
    tile_x = int(player.pos[0] // config.TILE_SIZE)
...
def on_shrine_check(game_state,sounds, screen, font, clock):
    # Handle tile pickup logic
        ...
        if 0 <= tile_x < config.TILE_MAP_SIZE and 0 <= tile_y < config.TILE_MAP_SIZE:
            current_tile = game_state.tile_map[tile_y][tile_x]
            if current_tile == 2:
                ...
                if tile_key not in has_picked_up:
                    ...
                    game_state.tile_map[tile_y][tile_x] = random.choice([0, 1])
                    
                    pause_duration = pygame.time.get_ticks() - pause_start_time
                    game_state.game_start_time = game_state.game_start_time + pause_duration

def handle_xp_orbs(gs, screen, camera_x, camera_y, font, clock, sounds):
    tile_x, tile_y = get_tile_x_y(gs.player)
    for orb in gs.xp_orbs[:]:
